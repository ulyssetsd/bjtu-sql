...
def like(cmt_id):
    if request.method == 'GET':
        ...
        cursor.execute("INSERT INTO like_cmt(cmt_id, user_id,c_time) VALUES(%s,%s,%s);", (cmt_id, session['logged_id'], c_time))
        conn.commit()
...
def unlike(cmt_id):
    if request.method == 'GET':
        cursor.execute("DELETE FROM like_cmt where cmt_id = %s AND user_id = %s;", (cmt_id, session['logged_id']))
        conn.commit()
        cursor.execute("SELECT msg_id FROM comment WHERE cmt_id = %s", (cmt_id,))
        c = cursor.fetchone()
    return redirect(url_for('message.show', msg_id=c['msg_id']))