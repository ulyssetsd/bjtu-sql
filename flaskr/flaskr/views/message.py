...
def show(msg_id):
    if request.method == 'GET':
        ...
        for c in cs:
            ...
            cursor.execute("SELECT * FROM like_cmt where cmt_id = %s AND user_id = %s", (c['cmt_id'], session['logged_id']))
...
def add():
    if request.method == 'POST':
        ...
        cursor.execute("INSERT INTO message(user_id,content,c_time) VALUES(%s,%s,%s);", (session['logged_id'], content, c_time))
        conn.commit()
...
def test():
    cursor.execute('SELECT * FROM message where user_id = %s ORDER BY c_time DESC', (session['logged_id'],))
    m = cursor.fetchall()
    print(m)